...
class AesService {
  ...
  AesService([String? encryptionKey]) {
    ...
    final keyString = encryptionKey ?? 'ZSLBTlLJf8yrOAojKmcYQTyA6aS4WK3Quv1yCWGU1rI/6VAQIh8y';
    final keyBytes = base64.decode(keyString);
    
    // Ensure key is exactly 32 bytes (256 bits) for AES-256
    if (keyBytes.length < 32) {
      throw ArgumentError('Encryption key must be at least 32 bytes when decoded');
    }
  ...
  String decrypt(String encryptedText) {
    ...
    try {
      final encryptService = Encrypter(AES(_key, mode: AESMode.cbc));
      final decryptedText = encryptService.decrypt(
