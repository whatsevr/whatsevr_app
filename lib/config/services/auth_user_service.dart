...
class AuthUserService {
    ...
    await otplessFlutterPlugin.openLoginPage(
      (result) {
        ...
        if (result['data'] != null) {
          ...
          final emailId = authServiceUserResponse.data?.identities
              ?.firstWhereOrNull(
                (element) => element.identityType == 'EMAIL',
              )
              ?.identityValue;
          final mobileNumber = authServiceUserResponse.data?.identities
              ?.firstWhereOrNull(
  ...
  static Future<void> loginToApp(
    ...
    final loginInfo = await AuthApi.login(
      userUid,
      mobileNumber,
      emailId,
    );
  ...
  static Future<void> getSupportiveDataForLoggedUser() async {
    print('53426327');
    final currentlyLoggedUserUid = AuthUserDb.getLastLoggedUserUid();
    try {
      if (currentlyLoggedUserUid != null) {
        final supportiveDataResponse =
